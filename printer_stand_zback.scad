// =======================================================================
// 3D Printer Stand — Z-TUBE BACK VARIANT
//
// Same as printer_stand.scad EXCEPT the back face is braced by two welded
// tube diagonals in a Z pattern (one in the lower bay, one in the upper),
// instead of a tensioned cable X.
//
// Units: inches.
//
// Two welded end-frames + 6 bolted long stretchers + Z-tube back diagonals
// (welded into the back-stretchers as a pre-assembled "back panel"). The
// back panel bolts to the legs through the same 5" plates as the other
// stretchers.
//
// Plywood shelves drop onto angle-iron ledgers. Wood top floats on slotted
// screws.
//
...
// Leveler = M12x1.75 swivel foot: 2" (50mm) rubber base + threaded rod + jam
// nut. Measured adjustable range (floor -> cap underside) is 42-72mm
// (1.65"-2.83"). LEVELER_HT is the leveler HARDWARE only (floor -> cap
// underside), NOT including the cap. Set to 2.0" (not the 2.25" range center)
// so the counter-end foot sits low enough that the opposite, down-slope end
// can still rise the ~1/2" the floor needs and stay under the 72mm max —
// see DESIGN.md "Height adjustment".
LEVELER_HT       = 2.0;    // nominal floor->cap-underside (leveler hardware only)
...
LEVELER_RANGE    = 0.5;    // ±0.5" to the hard stops; ~±0.34" usable with the
                           // 1/2" slope offset centered (doc)
FOOTCAP_THK      = 3/8;    // welded leg-end cap (1.5x1.5x3/8, tapped M12); stacks ABOVE the leveler
...
LEG_BOTTOM_Z = LEVELER_HT + FOOTCAP_THK;     // leg-tube bottom (cap stacks on leveler)
...
LEG_HT       = LEG_TOP_Z - LEG_BOTTOM_Z;     // nominal 33.4375 (2.0" lift + 3/8" cap)
...
module leveler(x, y) {
    ...
    // stud taps into it. The cap stacks ABOVE the leveler hardware — the leg
...
// === Z-TUBE BACK DIAGONALS ===================================================
// Two welded tube diagonals on the back face, in a Z pattern:
//   Upper diagonal: top-left to middle-right (slope \)
//   Lower diagonal: middle-left to bottom-right (slope /)
//
// Each diagonal lives in the plane of the back stretchers (Y = BACK_Y).
// Endpoints are at the inner-corner of each bay — i.e. where the diagonal
// meets the underside/topside of the relevant horizontal stretcher and the
// inner face of the back leg.
//
// In real fabrication, both diagonals weld to the back stretchers (NOT to
// the legs) so the entire back face — three back-stretchers + two diagonals
// — is pre-welded as a single "back panel". The panel bolts to the back
// legs through the existing six plate-pair joints (three stretchers × two
// ends each).

module zback_diagonals() {
    y = BACK_Y;
    x_left  = STRETCHER_END_LEFT;
    x_right = STRETCHER_END_RIGHT;

    // Upper bay: between top-stretcher underside and middle-stretcher topside
    upper_top_z = Z_TOP_STR - TUBE/2;
    upper_bot_z = Z_MID_STR + TUBE/2;
    // Lower bay: between middle-stretcher underside and bottom-stretcher topside
    lower_top_z = Z_MID_STR - TUBE/2;
    lower_bot_z = Z_BOT_STR + TUBE/2;

    // Upper diagonal: \  (top-left to middle-right)
    tube_between([x_left,  y, upper_top_z],
                 [x_right, y, upper_bot_z]);

    // Lower diagonal: /  (middle-left to bottom-right)
    tube_between([x_left,  y, lower_top_z],
...
//   Legs:                  4 × ~33.44"  of 1.5"×1.5"×0.120" tube (nominal at
//                            2.0" leveler lift + 3/8" cap; cut LONG, trim bottom to fit)
...
//                            stretcher tubes (welded to back stretchers,
